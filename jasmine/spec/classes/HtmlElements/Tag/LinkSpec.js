...
describe('Link-related functionality:', function() {
    ...
    describe('Link schemes', function(){
        var allowedSchemes;
        beforeEach(function(){
            allowedSchemes = {a: 'prefix a', b: 'prefix b', c: 'prefix c', d: 'prefix d'};
        });
        describe('Scheme validator', function(){
            it('returns false, if the scheme is not provided', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                expect(link.isValidScheme()).toBe(false);
            });
            it('returns true, if the scheme is the first one in array of allowed schemes', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                expect(link.isValidScheme('a')).toBe(true);
            });

            it('returns true, if the scheme is the last one in array of allowed schemes', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                expect(link.isValidScheme('d')).toBe(true);
            });
            it('returns true, if the scheme is in the middle of array of allowed schemes', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                expect(link.isValidScheme('c')).toBe(true);
            });

            it('returns false, if the array of allowed schemes is empty', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue({});
                expect(link.isValidScheme('c')).toBe(false);
            });

            it('returns false, if the scheme is not in the array of allowed schemes', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                expect(link.isValidScheme('f')).toBe(false);
            });

        });

        describe('Setting scheme', function(){
            it('sets the scheme if it is among allowed ones', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                link.setScheme('a');
                expect(link.getScheme()).toBe('a');
            });

            it('does not set the scheme if it is not among allowed ones', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                link.setScheme('not allowed scheme');
                expect(link.getScheme()).not.toBeDefined();
            });

            it('appends the scheme prefix if the scheme is set when href is already present', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                link.setHref('www.aaa.com');
                link.setScheme('a');
                expect(link.getHref()).toBe('prefix awww.aaa.com');
            });
            ...
            it('appends the scheme prefix to href only once', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                link.setHref('www.aaa.com');
                link.setScheme('b');
                link.setScheme('b');
                expect(link.getHref()).toBe('prefix bwww.aaa.com');
            });

            it('drops the previous scheme prefix', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                link.setHref('www.aaa.com');
                link.setScheme('a');
                link.setScheme('b');
            ...
            it('appends the scheme prefix if the scheme is set when href is not present', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                link.setScheme('a');
                link.setHref('www.aaa.com');
                expect(link.getHref()).toBe('prefix awww.aaa.com');
            ...
            it('appends the scheme prefix to href only once if href is not present', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                link.setScheme('b');
                link.setScheme('b');
                link.setHref('www.aaa.com');
            ...
            it('drops the previous scheme prefix if href is not present', function(){
                spyOn(link, 'getAllowedSchemes').and.returnValue(allowedSchemes);
                link.setScheme('a');
                link.setScheme('b');
                link.setHref('www.aaa.com');
