...
<row open><cell open><table open>"row 3"<table close><cell close><row close>\
    ...
    // xdescribe('Setter/getter for phantom object styles/attributes', function(){
    //     it('throws error if the method gets called without arguments', function(){
    //         expect(
    //             ).toThrow('Object name and its property name are required!');
    //     });
    //     it('throws error if first argument is neither cell, nor table nor row', function(){
    //         var invalids = ['string', 2, [], {dumb: 1}];
    //             invalids.forEach(function(invalid){
    //                 expect(function(){
    //                     table.getPhantomProperty(invalid, 'whatever');
    //                 }).toThrow("Allowed values for objects: cell, row, table");
    //             });
    //     });
    //     it('return undefined if phantom cell is not set', function(){
    //         expect(table.getPhantomProperty('cell', 'style')).not.toBeDefined();
    //     });
    // });

    // xdescribe('getBogusTableStyle(): gets bogus table style', function(){
