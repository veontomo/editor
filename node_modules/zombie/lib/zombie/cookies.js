...
Access = (function() {
  ...
  Access.prototype.all = function() {
    ...
    return this._cookies.all().filter(function(cookie) {
      var pathMatches;
      pathMatches = _this.path === "/" || Tough.pathMatch(_this.path, cookie.path);
      return Tough.domainMatch(_this.domain, cookie.domain) && pathMatches && cookie.TTL() > 0;
  ...
  Access.prototype.set = function(name, value, options) {
    ...
    pathMatches = this.path === "/" || Tough.pathMatch(this.path, cookie.path);
    if (Tough.domainMatch(cookie.domain, this.domain) && pathMatches && cookie.TTL() > 0) {
  ...
  Access.prototype.remove = function(name) {
    var _this = this;
    this._cookies.filter(function(cookie) {
  ...
  Access.prototype.update = function(serialized) {
    var cookie, pathMatches, _i, _len, _ref, _results;
    if (!serialized) {
      return;
    }
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      pathMatches = this.path === "/" || Tough.pathMatch(this.path, cookie.path);
      if (Tough.domainMatch(this.domain, cookie.domain) && pathMatches && cookie.TTL() > 0) {
