...
var Assert, BROWSER_OPTIONS, Browser, Console, Cookies, Credentials, EventEmitter, EventLoop, File, HTML, HTML5, Interact, MOUSE_EVENT_NAMES, Mime, Path, Q, Resources, Storages, URL, createTabs, extensionFunctions, format, ms,
...
BROWSER_OPTIONS = ["debug", "features", "headers", "htmlParser", "waitDuration", "proxy", "referer", "silent", "site", "userAgent", "maxRedirects"];
...
Browser = (function(_super) {
  ...
  function Browser(options) {
    var browser, extensionFunction, name, _i, _j, _len, _len1;
    if (options == null) {
      options = {};
    }
    browser = this;
    ...
    for (_j = 0, _len1 = extensionFunctions.length; _j < _len1; _j++) {
      extensionFunction = extensionFunctions[_j];
  ...
  Browser.prototype.visit = function(url, options, callback) {
    var deferred, resetOptions, site, _ref,
      _this = this;
  ...
  Browser.prototype.cookies = function(domain, path) {
    ...
    return this._cookies.access(domain, path || "/");
  };
  ...
  Browser.prototype.setCookie = function(name, value, options) {
...
Browser.extend = function(fn) {
  return extensionFunctions.push(fn);
};
