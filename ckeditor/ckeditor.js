						...
						for (; q;) {
							...
							if (q.type == CKEDITOR.NODE_COMMENT) q = q.getPrevious();
								...
								if (q.type == CKEDITOR.NODE_TEXT) {
									...
									k = q.getText();
									if (/[^\s\ufeff]/.test(k)) q = null;
									else for (var h = q.$.getElementsByTagName("*"), n = 0, C; C = h[n++];) if (!CKEDITOR.dtd.$removeEmpty[C.nodeName.toLowerCase()]) {
										q = null;
										break
									}
									q && (p = !! k.length)
	...
	(function() {
		...
		CKEDITOR.dom.selection.prototype = {
			...
			selectRanges: function(a) {
				...
				} else if (a.length == 1 && !a[0].collapsed && (b = a[0].getEnclosedNode()) && b.type == CKEDITOR.NODE_ELEMENT && b.getAttribute("contenteditable") == "false") this.fake(b);
				else {
					if (g) {
